...
save_path = "./plots/uber incentive vs regret.pdf"
save_path2 = "./plots/uber incentive vs c.pdf"
save_path3 = "./plots/uber incentive vs c two box.pdf"
save_path4 = "./plots/uber incentive vs forecast error.pdf"
save_path5 = "./plots/uber incentive vs forecast.pdf"
...
fig, ax = LineChart([df.loc[df["Scenario"]=='Optimal Expectation']["Total $\\rho$"], df.loc[df["Scenario"]=='Optimal Expectation']["Total $\\rho$"]], [df.loc[df["Scenario"]=='Optimal Expectation']["Regret"], df.loc[df["Scenario"]=='Uniform Expectation']["Regret"]]
            ...
            , save_path="./plots/uber incentive vs regret no box.pdf") # , ylim=(0, 200)
...
fig.clf()
fig, ax = plt.subplots()
S_gt = np.load("./uber numpy data/Sgt0.npy").reshape(-1)
x = np.arange(0, len(S_gt), 1)
ax.plot(x, S_gt, color='black', linestyle='-.')

for rho, marker, op in zip([0, 3, 5], ['o', '+', '^'], [0.8, 0.6, 0.4]):
    S_hat = np.load("./uber numpy data/S_hat0_rho{}.npy".format(rho)).reshape(-1)
    uni_S_hat = np.load("./uber numpy data/uni_S_hat0_rho{}.npy".format(rho)).reshape(-1)

    ax.plot(x, S_hat, color=color_list[2], linestyle='-', marker=marker, markersize=6, alpha=op)
    ax.plot(x, uni_S_hat, color=color_list[0], linestyle='--', marker=marker, markersize=6, alpha=op)
